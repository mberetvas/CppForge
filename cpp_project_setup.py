...
def create_vscode_settings(path: Path) -> bool:
    """
    Create a .vscode/settings.json file with default settings.
...
"""
    ...
    if not create_directory(vscode_dir):
        return False
    return create_file(vscode_dir / "settings.json", settings_content)


def setup_cpp_project(target_project_directory: Path):
    """
    ...
    Args:
        target_project_directory (Path): The path where the project should be created
    """
    ...
    root_dir = target_project_directory / project_name
...
"""
    ...
    if not create_vscode_settings(root_dir):
        logging.warning("Failed to create .vscode/settings.json file. Continuing with setup...")
